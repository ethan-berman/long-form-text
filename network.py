...
def clean_tweet(tweet):
   #improve me, i make ugly text and I get rid ofthe wrong characters
	...
	cutDash = re.sub("--", " ", cutAts)
	cutHyphen = re.sub("-", " ", cutDash)
	letters = re.sub("[^a-zA-Z-#]", " ", cutHyphen)
	words = letters.lower().split()
	stops = set(stopwords.words("english"))
	good_words = [w for w in words if not w in stops]
	#print(good_words)
	good_words = [stemmer.stem(w.lower()) for w in good_words if w not in ignore_words]
...
def makebagofwords(pruned):
	...
	for prune in pruned:
		...
		bowtraining1.append(tempbag)


		inputs = {'inputs': bowtraining1}
		inputs_file = "inputs.json"

	with open(inputs_file, 'w') as outfile:
		json.dump(inputs, outfile, indent=4, sort_keys=True)
...
#print("this is outputs" +str(output))
#pruned is a list of training data
inputs_file = 'inputs.json' 
with open(inputs_file) as data_file: 
	inputs = json.load(data_file) 
	bowtraining = np.asarray(inputs['inputs']) 

outputs_file = 'outputs.json' 
with open(outputs_file) as data_file: 
	outputs = json.load(data_file) 
	output = np.asarray(outputs['outputs']) 
